...
"""
# ============================================
# IMPORTS DES BIBLIOTHÈQUES ET SIMULATION
# ============================================
...
# Essai d'importation de la vraie librairie Raspberry Pi
try:
    from grove.grove_bme280 import BME280
    SIMULATION = False
except ImportError:
    # Si on est sur un PC normal, on crée un faux capteur (Mock)
    print("⚠️ AVERTISSEMENT : Matériel non détecté.")
    print("🔄 Passage en MODE SIMULATION (Génération de fausses données).")
    SIMULATION = True
    
    class BME280:
        """ Faux capteur BME280 pour tester le code sans le matériel """
        def __init__(self):
            pass
            
        @property
        def temperature(self):
            # Génère une température entre 28 et 38°C (pour déclencher tes alertes)
            return random.uniform(28.0, 38.0)
            
        @property
        def humidity(self):
            # Génère une humidité entre 25% et 90%
            return random.uniform(25.0, 90.0)
            
        @property
        def pressure(self):
            # Génère une pression en Pascal (le vrai capteur donne des Pa)
            return random.uniform(100000.0, 102500.0)
